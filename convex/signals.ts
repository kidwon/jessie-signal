...
export const snapshot = internalAction({
  ...
  handler: async (ctx) => {
    ...
    // Admin config: which target states are allowed to fire an alert email.
    const settings = await ctx.runQuery(internal.settings.read, {})

    // Alert on a scenario change when the new scenario is enabled in settings.
    const to = data.scenario.scenario
    const from = prev?.scenario
    if (from != null && to !== from && settings.scenarios.includes(to)) {
    ...
    // Alert on a Fear & Greed level change when the new level is enabled.
    if (prev != null) {
      const fgFrom = fgLabel(prev.fg_score).label_en
      const fgTo = data.fear_greed.label_en
      if (fgTo !== fgFrom && settings.fearGreed.includes(fgTo)) {
        await ctx.scheduler.runAfter(0, internal.notify.notifyFearGreedChange, {
          label_zh: data.fear_greed.label_zh,
          label_en: data.fear_greed.label_en,
